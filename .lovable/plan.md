...
### 7. Vendor portal
_DONE (PM-side comparison + evaluation surface)._ Bug C (vendor-portal RFQ completeness) was already shipped in Batch 1; remaining scope for Item 7 was the **procurement-side comparison + evaluation surface** on the RFQ Compare Quotations dialog. Implemented in `src/components/RFQManagement.tsx`:
- Header now surfaces the **RFQ submission deadline** (Clock icon) alongside the quote count.
- New **Cards / Side-by-side** view toggle in the Compare dialog header.
- **Per-submission terms** rendered inline on each Card (Payment Terms, Validity, Warranty) so PMs no longer have to open the detail dialog to see each vendor's commercial offer.
- New **Evaluation block** per submission — multi-line internal notes textarea + 0–10 score input + Save button, wired to new `quotationEvaluationApi.save()` (`PUT /api/quotations/{id}/evaluation`).
- New **Side-by-side table view** for compact head-to-head comparison: vendor, price, delivery, valid-until, payment terms, validity, warranty, overall score, evaluation snippet, Award action.
- Evaluation drafts seed from `evaluation_notes` / `evaluation_score` on the fetched quotation payload so reloads preserve prior decisions.

**Backend ask (BLOCKING):** `PUT /api/quotations/{id}/evaluation` accepting `{ evaluation_notes?: string, evaluation_score?: number|null }` (procurement role). Persist on the quotations row and include `evaluation_notes`, `evaluation_score`, `evaluation_updated_at` in every quotation payload (`GET /api/rfqs/{id}/quotations`, `GET /api/quotations/rfq/{id}`). Score is a 0–10 decimal (allow 0.5 step). See `frontend_changes.md` § Batch 2 — Item 7.
...
_DONE._ Audit confirmed driver phone/licence/email-optional, vehicle edit form, and maintenance module (`VehicleMaintenanceTab` + `UpcomingMaintenanceWidget`) were already shipped. Remaining gap was **driver-level document storage** — addressed by new `driversApi.{listDocuments,uploadDocument,deleteDocument}`, new `DriverDocumentsDialog.tsx` (upload + table with the same Valid/Expiring/Critical/Expired tier colouring as `VehicleDocumentsTab`), and a `FileText` action in `DriverManagement` row menu. Backend asks (BLOCKING): `GET/POST /api/fleet/drivers/{id}/documents` and `DELETE /api/fleet/drivers/{id}/documents/{doc_id}` per `frontend_changes.md` § Batch 2 — Item 9.
