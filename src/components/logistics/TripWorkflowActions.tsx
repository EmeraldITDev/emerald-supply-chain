...
export function TripWorkflowActions({ trip, userRole, onUpdated }: TripWorkflowActionsProps) {
  ...
  const selectedVendorId =
    (trip as Trip & { selected_vendor_id?: string }).selected_vendor_id ||
    trip.vendorId;
  ...
  const loadConvertOptions = async () => {
    ...
    if (vRes.success && vRes.data) {
      ...
      setVendors(arr.map((v: { id?: string | number; name?: string; company_name?: string }) => ({
        id: String(v.id),
  ...
  return (
    <div className="space-y-3 rounded-lg border p-4 bg-muted/30">
      ...
      {stage === "trip_request" &&
        (userRole === "logistics_manager" || userRole === "logistics_officer" || userRole === "admin") && (
          <Button
            size="sm"
            disabled={busy}
            onClick={() => {
      ...
      {stage === "scd_approval" &&
        (userRole === "supply_chain_director" || userRole === "supply_chain") && (
          <Button
            size="sm"
            disabled={busy}
            onClick={() =>
      ...
      <Dialog open={convertOpen} onOpenChange={setConvertOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Assign vendor, vehicle, passengers, and driver.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button
              disabled={busy || !vendorId || !vehicleId || !driverUserId}
              onClick={async () => {
                setBusy(true);
                try {
                  const res = await tripRequestApi.convertToLogisticsRequest(String(trip.id), {
